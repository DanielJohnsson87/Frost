...
*/
  public function Manage($action=null) {
    ...
    try {
      ...
      $this->db->ExecuteQuery(self::SQL('insert content'), array('htmlpurify', 'page', 'htmlpurify', "This is a demo page with some HTML code intended to run through <a href='http://htmlpurifier.org/'>HTMLPurify</a>. Edit the source and insert HTML code and see if it works. \n\n<b>Text in bold</b> and <i>text in italic</i> and <a href='http://dbwebb.se'>a link to dbwebb.se</a>. JavaScript, like this: <javascript>alert('hej');</javascript> should however be removed.", 'htmlpurify', $this->user['id']));
      $this->db->ExecuteQuery(self::SQL('insert content'), array('phpmarkdown', 'page', 'phpmarkdown', 
  ...
  */
  public function Save() {
  	$msg = null;
  	if($this['id']) {
  		$this->db->ExecuteQuery(self::SQL('update content'), array($this['key'], $this['type'], $this['title'],
  			$this['data'], $this['filter'], $this['id']));
  		$msg = 'update';
  	} else {
  		$this->db->ExecuteQuery(self::SQL('insert content'), array($this['key'], $this['type'], $this['title'],
  			$this['data'], $this['filter'], $this->user['id']));
  		$msg = 'created';
  	}
  ...
  *
  * @param $data string of text to filter and format according to it's filter settings.
  ...
  */
  public static function Filter($data, $filter) {
    switch($filter) {
      ...
      case 'phpmarkdown': $data = CPHPMarkdown::PHPMarkdown($data); break;
      case 'plain':
  ...
  /**
  * Get thw filtered content.
  *
  * @return string with the filterd data.
  */
