...
class CMUser extends CObject implements IHasSQL, ArrayAccess, IModule {
  ...
  */
  public function Create($acronym, $password, $name, $email) {
    ...
    $this->db->ExecuteQuery(self::SQL('insert into user'), array($acronym, $name, $email, 
      $pwd['algorithm'], $pwd['salt'], $pwd['password']));
  ...
  * @param $algorithm string stating what algoruthm to use, plain, md5, md5salt, sha1, sha1salt.
  * defaults to the settings of site/config.php
  * @return array with 'salt' and 'password'.
  */ 
  public function CreatePassword($plain, $algorithm=true) {
      $password = array(
      'algorithm'=>($algorithm ? CLydia::Instance()->config['hashing_algorithm'] : $algorithm),
      'salt'=>null
    );
              switch($password['algorithm']) {
                case 'sha1salt': $password['salt'] = sha1(microtime()); $password['password'] = sha1($password['salt'].$plain); 
                break;
